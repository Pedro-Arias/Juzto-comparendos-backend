...
class ComparendoVerifik(IVerifik):
    ...
    def __init__(self) -> None:
        ...
        self.__comparendos_obj = {'comparendos': list(), 'resoluciones': list(), "scrapper_errors": list()}
    ...
    async def get_infractions(self, customer: Profile) -> dict:
        ...
        endpoints = ['scraper-bogota', 'scraper-simit']
        payloads = [
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for fn, pl in zip(endpoints, payloads):
                future = executor.submit(self.get_comparendos_url, fn, pl)
    ...
    def _save_infractions(self, customer: Personas) -> bool:
        ...
        if (isinstance(customer, Personas) and 
            ...
            if len(data_bd) > 0 and not self.__comparendos_obj.get("scrapper_errors"):
            ...
            if len(data_api)>0:
                ...
                if self.__comparendos_obj.get("scrapper_errors"):
                    logs_personas = {
                        'origen': self.__customer._origin,
                        'resultado': 'Error en los scrappers',
                        'fecha': IUtility.datetime_utc_now(),
                        'id_persona': customer
                    }
                    Logs_personas.objects.create(**logs_personas)
                    raise Exception(self.__comparendos_obj.get("scrapper_errors"))
                    saved = False
                else:
                    logs_personas = {
                        'origen': self.__customer._origin,
    ...
    def set_comparendos(self, resoluciones, comparendos):
        self.__comparendos_obj['comparendos'] = comparendos
        self.__comparendos_obj['resoluciones'] = resoluciones